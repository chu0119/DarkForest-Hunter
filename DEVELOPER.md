...
**watch 主路径**（`run.py watch` → `watch_tui.py` + `scanner_engine.py`）：
1. 每个数据源独立线程循环：github_search 走 `QueryRotator`（查询轮转 + 收益排序 + 新鲜簇）→ `ScannerEngine._scan_one_query()` 从 `text_matches` 提取候选 key，流式 submit 到 `VerificationBroker`。
2. 外部源（gitlab/npm/commits/events...）经 `ScannerEngine.run_multi_source()` → `_get_scanner_registry()` 映射到 Scanner 类异步执行。
3. Broker worker 用 `UnifiedKeyVerifier` 验证：先 GET models/balance（只读），通过后一次 `max_tokens=1` 最小探测确认实际可用（`allow_chat_probe` 默认开）；`models_unauthenticated` 平台（/models 公开）由 `probe_unclear` 兜底探测。
4. 结果落 SQLite（`store.upsert`）+ watch_state/CSV 台账；`QueryTracker` 按 valid/高价值转化回写查询收益，驱动排序与质量熔断。

**编程调用路径**（`scanner_engine.ScannerEngine.run()`）：
- `build_active_queries()` 合并静态查询 + 动态滚动时间窗口；`engine.run(queries)` 逐条 Code Search → `_verify_dict()` 验证。保留用于测试与程序化调用，CLI 已统一走 watch。
...
| 模块 | 职责 | 关键导出 |
|------|------|----------|
| `run.py` | 统一 CLI 入口（watch 唯一扫描入口 + maintenance/metrics 工具） | `cmd_watch` / `cmd_maintenance` / `cmd_metrics` |
| `scanner_engine.py` | 扫描引擎。维护查询库、执行 GitHub 搜索、text_matches 提取、验证、保存 | `ScannerEngine`、`BUILTIN_QUERIES`、`generate_rolling_time_queries`、`build_active_queries` |
| `providers.py` | 35 家 AI 平台配置 + 统一匹配/验证器 | `AIProvider`、`ALL_PROVIDERS`、`PROVIDER_MAP`、`UnifiedKeyMatcher`、`UnifiedKeyVerifier`、`DIRECT_PROVIDERS`、`USD_PROVIDERS` |
| `scanners/base.py` | 所有扫描器的抽象基类。提供 key 正则、坏 key 过滤、`_get_with_retry`（429 退避） | `BaseScanner`、`extract_keys`、`is_bad_key` |
| `scanners/*.py` | 30 个具体扫描器，各管一个数据源 | 见 `scanners/__init__.py` |
...
- `_verify_dict(keys_dict)` — 用 `UnifiedKeyVerifier` 做多平台验证并查余额（默认 GET-only，不消耗配额），折算成 USD/CNY。
- `_persist_results(results)` — 把单次扫描的 valid/invalid 结果写入 SQLite，保留转化率证据。
- `QueryTracker.record_outcome(...)` / `get_quality_yield(...)` — 按验证后的 valid 和高价值结果动态调整查询排序。
...
### 1. 优化查询集 `queries_optimized.txt`（约 150 条，优先加载）

位于项目根目录，是手工调优的高产出 GitHub Code Search 语句（`build_active_queries()`
优先读取它，找不到时才回退到 `scanner_engine.py` 内的 `BUILTIN_QUERIES`）。例如：
```
...
- 最终得到 **155 条**活跃查询。每次扫描自动指向最近 7/30 天的提交，逆转了"固定日期查询随时间产出下降"的问题。
